      ...
      <section className="px-8 py-20">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
          ...
          <div className="lg:col-span-3 space-y-8">
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-cyan/10 border border-cyan/30">
              <span className="text-cyan text-sm font-medium">Powered by Hyperledger Fabric</span>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/auth/register?role=msme" className="btn-primary text-lg px-8 py-4">
                Get Started as MSME
              </Link>
              <Link to="/auth/register?role=lender" className="btn-secondary text-lg px-8 py-4">
                Lender / NBFC Access
              </Link>
              <Link to="/auth/register?role=regulator" className="text-cyan hover:text-cyan-dark transition-colors text-lg px-8 py-4 flex items-center space-x-2">
...
function StatsBar() {
  ...
  return (
    <section className="px-8 py-12 bg-navy-light border-y border-navy-lighter">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="text-center">
          <p className="text-4xl font-display font-bold text-cyan mb-2">₹{counts[0]} Cr</p>
          <p className="text-gray-400">Protected from duplicate financing</p>
        </div>
        <div className="text-center">
          <p className="text-4xl font-display font-bold text-cyan mb-2">{counts[1].toLocaleString()}+</p>
          <p className="text-gray-400">Invoices on-chain</p>
        </div>
        <div className="text-center">
          <p className="text-4xl font-display font-bold text-cyan mb-2">{counts[2]}ms</p>
          <p className="text-gray-400">Average verification time</p>
        </div>
        <div className="text-center">
          <p className="text-4xl font-display font-bold text-emerald mb-2">{counts[3]}</p>
          <p className="text-gray-400">Duplicate financing incidents</p>
        </div>
      </div>
    </section>
  );
...
function ArchitectureDiagram() {
  ...
  return (
    <section className="px-8 py-20 bg-navy-light">
      <div className="max-w-7xl mx-auto">
        <h2 className="font-display text-5xl font-bold text-center mb-4">
          Hybrid Audit Architecture
        </h2>
        <p className="text-center text-gray-400 mb-16">Hover over each layer to learn more</p>
          ...
          {layers.map((layer, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display text-xl font-bold mb-1">{layer.name}</h3>
                  <p className="text-gray-400 text-sm">{layer.desc}</p>
                </div>
                <div className={`w-3 h-3 rounded-full ${hoveredLayer === i ? `bg-${layer.color}` : 'bg-gray-600'}`}></div>
              </div>
            </div>
          ))}
        </div>
...
function Footer() {
  return (
    <footer className="px-8 py-12 border-t border-navy-lighter">
      <div className="max-w-7xl mx-auto">
